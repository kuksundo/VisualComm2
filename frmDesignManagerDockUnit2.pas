{
  ...
  **)E2201: Need imported data reference에러 발생시 대처 방법:
  ...
  1) bpl로 부터 Component load함
     ...
     -       각 package의 requires에 pjhPackageUnits.dcp 추가 후 build
...
//    FEaster_Start: TEasternEgg;//HotKey 수신용(Ctrl+debug)
    ...
    FDesignDebugMode, //DesignManager_Debug.exe로 실행하면 True
    FNoUsePackageMode //HiMECSWatch2_NoBpl.exe 실행하면 True
    : Boolean;
...
begin
  ...
  else//DesignManager_Debug.exe일때 실행됨
  begin
  end;
end;
...
{$IFDEF NOUSE_PACKAGE}
      //이 Mode로 실행하면 "TpjhTPanel already exist" 에러가 발생하므로 LoadPackage Skip함
