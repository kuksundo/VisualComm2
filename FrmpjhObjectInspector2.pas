...
type
  ...
  public
    ...
    FIsOnInsert: Boolean; //컨트롤이 삽입되는 동안 True
  ...
  begin
    ...
    begin
      if Length(AValue) > 255 then
        AValue := 'Length is over 255!'; //Watch2에서 컴포넌트,프로퍼티 이름으로 Value를 가져옴
...
//          LStr := TJSON.Stringify<TpjhTagInfo>(TpjhTagInfoPropEditor(Sender).pjhTagInfo4Editor);
          //TpjhTagInfoPropEditor.Edit함수에서 TELPropertyInspectorItem(PropInspItem).Expand를 실행해야
          //PropInsp.ActiveItem.Items[i]에 데이타가 채워짐
...
//          ShowMessage(FSelectedControlName + ' : ' + PropInsp.ActiveItem.Caption + ' : ' + PropInsp.ActiveItem.DisplayValue);
        ...
        //변경된 Value를 Watch2에 전달함(CommandJson을 만들기 위함)
